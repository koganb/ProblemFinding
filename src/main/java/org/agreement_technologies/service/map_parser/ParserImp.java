...
public class ParserImp implements PDDLParser {
    ...
    private TaskImp.NumericExpressionImp parseNumericExpression(SynAnalyzer syn, TaskImp task, 
        ...
        if (token.isSym(Symbol.SS_NUMBER)) {
            ...
            if (token.isSym(Symbol.SS_NUMBER)) {
                ...
                switch (token.getSym()) {
                    case SS_PLUS: exp = task.new NumericExpressionImp(TaskImp.NumericExpressionType.NET_ADD);   break;
                    case SS_DASH: exp = task.new NumericExpressionImp(TaskImp.NumericExpressionType.NET_DEL);   break;
                    case SS_MULT: exp = task.new NumericExpressionImp(TaskImp.NumericExpressionType.NET_PROD);  break;
                    case SS_DIV:  exp = task.new NumericExpressionImp(TaskImp.NumericExpressionType.NET_DIV);   break;
                }
    ...
    private TaskImp.CongestionPenaltyImp parseCongestionPenaltyExpression(SynAnalyzer syn, TaskImp task, TaskImp.CongestionImp congestion) throws ParseException {
        ...
        switch (token.getSym()) {
            case SS_EQUAL:      penalty = task.new CongestionPenaltyImp(TaskImp.ConditionType.CT_EQUAL);      break;
            case SS_GREATER:    penalty = task.new CongestionPenaltyImp(TaskImp.ConditionType.CT_GREATER);    break;
            case SS_GREATER_EQ: penalty = task.new CongestionPenaltyImp(TaskImp.ConditionType.CT_GREATER_EQ); break;
            case SS_LESS:       penalty = task.new CongestionPenaltyImp(TaskImp.ConditionType.CT_LESS);       break;
            case SS_LESS_EQ:    penalty = task.new CongestionPenaltyImp(TaskImp.ConditionType.CT_LESS_EQ);    break;
            case SS_DISTINCT:   penalty = task.new CongestionPenaltyImp(TaskImp.ConditionType.CT_DISTINCT);   break;
        }
        syn.openPar();
