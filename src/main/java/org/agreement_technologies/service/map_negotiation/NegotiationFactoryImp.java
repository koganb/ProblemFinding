...
public class NegotiationFactoryImp implements NegotiationFactory {
	...
	switch (negotiationType) {
		case COOPERATIVE:   ps = new CooperativePlanSelection(c, st);   
                                    break;
		case BORDA:         ps = new CustomBordaNegotiation(c, st, BORDA_PROPOSALS);  
                                    break;
                case RUNOFF:        ps = new RunoffVoting(c, st);  
                                    break;
		default:            ps = null;
		}
        return ps;
